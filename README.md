...
SAS‑FAS 是一套面向 AI Agent 的长期投资研究 Skill。它把监管文件、财务数据、链上状态、协议机制、市场结构与用户资料，整理为可追溯的证据账本，并在相互隔离的 Bull / Bear 论证之后，形成情景估值、风险判断与可执行裁决。
...
- **政策与预测分层**：可执行仓位规则和不确定的市场情景分开呈现，互不借用确定性。
- **情景权重透明**：没有统计基础时只称“主观情景权重”，并测试不同权重下行动是否稳定。
- **触发器资产隔离**：单一资产的估值信号只能触发该资产；跨资产配置需要接收资产自己的证据。
- **负面证据分级**：明确区分“已有证据显示恶化”和“尚无足够证据支持乐观结论”。
...
06_judge.json             # 可机器校验的评级、情景权重、稳健性与建议模式
...
| 决策 | 证据等级、基本盘状态、赔率状态、情景稳健性、分阶段仓位、资产隔离触发器与做空边界 |
